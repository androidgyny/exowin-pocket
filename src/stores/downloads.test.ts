...
function makeProgress(overrides: Partial<{
  ...
  extras_done: boolean; extras_progress: number;
}> = {}) {
  return {
    file_index: 0,
    file_name: "game.zip",
...
describe("downloads state machine", () => {
  ...
  it("allows only one progress poll at a time", async () => {
    let progressCalls = 0;
    let releasePoll: ((value: ReturnType<typeof makeProgress>) => void) | null = null;
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === "download_game") { return Promise.resolve("ok"); }
      if (cmd === "get_download_progress") {
        progressCalls += 1;
        ...
        return Promise.resolve(makeProgress({ progress: 0.2 }));
      }
      return Promise.resolve(null);
    });

    const { startGameDownload } = await import("./downloads");
    startGameDownload(131);

    vi.advanceTimersByTime(5100);
    await Promise.resolve();
    expect(progressCalls).toBe(1);

    expect(releasePoll, "the first poll should still be pending").not.toBeNull();
    releasePoll!(makeProgress({ progress: 0.1 }));
    await Promise.resolve();
    await Promise.resolve();
    await vi.advanceTimersByTimeAsync(1100);
  ...
  it("resumes extras tracking without restarting an installed game download", async () => {
    mockInvoke.mockImplementation((cmd: string) => {
      if (cmd === "get_download_progress") {
        return Promise.resolve(makeProgress({
          progress: 1,
          ...
          installed: true,
          extras_done: false,
          extras_progress: 0.25,
        }));
      }
      return Promise.resolve(null);
    });

    const { watchExtrasIfPending, getDownloadState, stopGameDownloadTracking } = await import("./downloads");
    await watchExtrasIfPending(132, "Installed Game");

    expect(getDownloadState(132)).toMatchObject({
      status: "Installed - checking extras...",
      installed: true,
      downloading: false,
    });
    expect(mockInvoke).not.toHaveBeenCalledWith("download_game", expect.anything());

    await vi.advanceTimersByTimeAsync(1100);
    expect(getDownloadState(132)?.status).toContain("downloading extras");
    expect(mockInvoke).not.toHaveBeenCalledWith("download_game", expect.anything());
    stopGameDownloadTracking(132);
  });
