import { onMount, onCleanup, For, Show, createSignal, createMemo, createEffect, untrack } from "solid-js";
...
export function Library() {
  ...
  createEffect(() => {
    ...
    if (!change) { return; }
    // This effect belongs only to the library-change event. Reading
    // detailGame/userPlaylists here used to subscribe it to those signals;
    // setting a fresh detail object then retriggered the effect and produced
    // an endless shelf-refresh loop.
    untrack(() => {
      refreshRecent();
      refreshInstalled();
      refreshFavorites();
  ...
  onMount(() => {
    ...
    // Favorites are updated locally when the user clicks the star, so polling
    // the full grouped catalogue for them every five seconds only burns CPU.
    const interval = setInterval(() => { refreshRecent(); refreshInstalled(); }, 5000);
