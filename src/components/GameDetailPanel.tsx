...
export function GameDetailPanel(props: Props) {
  ...
  const rawNote = (): PanelNote | null => {
    ...
    const source = selected()?.torrent_source ?? props.game?.torrent_source;
    if (navigator.userAgent.includes("Android") && source === "eXoWin3x") {
      return {
        key: "android-exowin-core-options",
        text: "Some Windows games need different DOSBox Pure settings. If a game does not start "
          + "correctly, adjust its CPU type, memory, or performance in RetroArch's Quick Menu → Core Options.",
      };
    }
