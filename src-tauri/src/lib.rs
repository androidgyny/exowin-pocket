...
fn materialize_android_resources(root: &Path) -> Result<(), String> {
    ...
    write(
        &metadata.join("Win3x.xml.gz"),
        include_bytes!("../../metadata/Win3x.xml.gz"),
    )?;
    write(
        &metadata.join("Win3x_configs.zip"),
        include_bytes!("../../metadata/Win3x_configs.zip"),
    )?;
    write(
        &metadata.join("dosbox3x.txt"),
        include_bytes!("../../metadata/dosbox3x.txt"),
    )?;
    write(
        &torrents.join("eXoWin3x.torrent"),
        include_bytes!("../../torrents/eXoWin3x.torrent"),
    )?;
...
fn init_logger(log_dir: &std::path::Path) -> Option<std::path::PathBuf> {
    ...
    // Bridge `log!` â†’ tracing so log-only crates land in the same sink.
