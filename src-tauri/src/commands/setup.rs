...
pub struct CollectionDef {
    ...
    /// e.g. "eXo/eXoDOS" ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ games are at <inner_folder>/eXo/eXoDOS/<shortcode>/
    pub game_prefix: &'static str,
    /// Segment in the LaunchBox application_path used to extract the shortcode.
    /// e.g. "!dos" for eXoDOS (path looks like "eXo\eXoDOS\!dos\<shortcode>\ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦")
...
/// dir, so a legacy install looks like `<data>/eXo/ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦` + `<data>/Content/ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦`.
...
///
/// Replaces the old per-collection roots (`<data>/eXoWin9x/ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦`), which were an
...
pub async fn get_available_collections(
    ...
    Ok(COLLECTION_MAP
        .iter()
        .filter(|c| !cfg!(target_os = "android") || c.id == "eXoWin3x")
...
/// "ResolvingÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦" forever in shipped Windows builds - by doing both lookup
...
fn torrent_root_looks_empty(torrent_root: &Path) -> bool {
    let iter = match std::fs::read_dir(torrent_root) {
        Err(_) => return true, // doesn't exist or unreadable ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ safe to seed
        Ok(it) => it,
    };
...
/// fresh install starts in seconds instead of 5ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ10 minutes on Windows.
...
pub async fn init_download_manager(
    ...
    // ExoWin Pocket Android manager override
    // Setup already stores eXoWin3x, but force the dedicated Android build to
    // that one collection so stale/default eXoDOS state can never win.
    #[cfg(target_os = "android")]
    let collections: Vec<&str> = vec!["eXoWin3x"];
    ...
    // ExoWin Pocket requires the Win3x manager. The inherited API returns a
    // bool, and Setup historically ignored false, which allowed first-run
    // setup to "succeed" with no manager and fail only when Play was pressed.
    #[cfg(target_os = "android")]
    {
        let has_win3x = torrent_state.0.read().await.contains_key("eXoWin3x");
        if !has_win3x {
            return Err(
                "eXoWin3x download manager failed to initialize. Check the app log for the underlying torrent/session error."
                    .to_string(),
            );
        }
    }
...
///
/// Sharing is OPT-IN: only an explicit "1" permits piece-data uploads; anything
/// else (including an unset key) keeps uploads blocked. Uploading copyrighted
...
/// Checks: dev project dir ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ data_dir/thumbnails ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ exe dir/thumbnails
#[tauri::command]
...
pub async fn get_preview_dir(collection: String) -> Result<String, String> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(exe_dir) = exe.parent() {
            ...
            // Linux /usr layout: /usr/bin/exodium ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ /usr/lib/exodium/previews/
...
pub async fn get_default_data_dir() -> Result<String, String> {
    #[cfg(target_os = "android")]
    {
        ...
        return Ok("/storage/emulated/0/ExoWinPocket".to_string());
    }
...
pub async fn setup_from_local(
    ...
    if let Ok(metadata_dir) = bundled_metadata_dir() {
        ...
        let conn = db_state.0.lock().map_err(|e| e.to_string())?;
        // Every LPÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂEN match is family-scoped (CLAUDE.md ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â§1): titles and
        ...
        // the same game (e.g. PL "Legend of Kyrandia Book 2" ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â EN
...
fn scan_installed_games_with_db(
    ...
    for col in COLLECTION_MAP {
        ...
        // Build "UPDATE ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦ WHERE shortcode IN (?, ?, ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦) AND torrent_source = ?"
    ...
    // Pass 2: detect downloaded-but-not-extracted game ZIPs ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ mark as installed + in_library.
    ...
    if !cfg!(target_os = "android") && game_base.is_dir() {
        // Build lookup: zip_stem ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ game_id, restricted to non-LP collections.
...
pub async fn scan_installed_games(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        scan_installed_games_with_db(&db, &data_dir)
    })
    .await
    .map_err(|e| format!("installed-game scan task failed: {e}"))?
}
