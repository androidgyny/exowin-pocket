...
impl Session {
    ...
    /// Enable or disable piece-data uploads without stopping peer discovery or
    /// downloads. Existing torrents observe this value immediately.
    #[cfg(feature = "disable-upload")]
    pub fn set_upload_disabled(&self, disabled: bool) {
        self._disable_upload.store(disabled, Ordering::Release);
    }

    #[cfg(feature = "disable-upload")]
    pub fn upload_disabled(&self) -> bool {
        self._disable_upload.load(Ordering::Acquire)
    }
    ...
    pub fn new_with_opts(
        ...
        async move {
            ...
            let session = Arc::new(Self {
                ...
                #[cfg(feature = "disable-upload")]
                _disable_upload: AtomicBool::new(opts.disable_upload),
                blocklist,
                allowlist,
                lsd,
            });
...
mod tests {
    ...
    #[cfg(feature = "disable-upload")]
    #[tokio::test]
    async fn upload_disable_can_be_toggled_on_a_live_session() {
        ...
        let session = Session::new_with_opts(dir.path().to_path_buf(), options)
            .await
            .unwrap();

        assert!(session.upload_disabled());
        session.set_upload_disabled(false);
        assert!(!session.upload_disabled());
        session.set_upload_disabled(true);
        assert!(session.upload_disabled());
    }
