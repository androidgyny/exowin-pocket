...
## ExoWin Pocket v0.13.1-pocket.1 - 2026-08-23

### Added

- Initial public ExoWin Pocket Android MVP.
- Curated eXoWin3x browser, selective downloads, favorites, uninstall support,
  optional artwork and metadata packs, and RetroArch/DOSBox Pure launching.
- Shared Windows 3.1 DOSZ launch translation for ordinary folder, batch, CD,
  floppy, multi-disc, custom Windows tree, and drive-letter recipes.
- Android storage permission and RetroArch launch bridges.
- Per-game DOSBox Pure option seeding for a small number of validated titles,
  without replacing user-created option files.
- In-app warning that Windows games may require RetroArch core-option changes.

### Known limitations

- Requires a user-built `Windows311-EXOWIN.dosz`; no Windows files are bundled.
- Compatibility is incomplete and only a small part of the catalog has been
  tested manually.
- Nineteen recipes with known blockers are excluded from the Android catalog.
